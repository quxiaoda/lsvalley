...
import { cookie, getPlatform} from './base'
...
var get_track_common = { //配置默认需要统计的参数、获取客户端支持的所有的协议
    ...
    page_wapapp: function(page_platform) {
        ...
        //新老用户标示，0：新用户，1：老用户
        var user_type = cookie.get("user_type") || "-1";
        //获取用户身份
        var user_role = cookie.get("user_role") || "-1";
        //需要存入cookie的字段
        param_arr = {
            "source": "tao800_wap",
            "jump_source": 2,
...
trackLog.trackOutstr = function(track_data) { //需要统计的参数拼接
    ...
    _.extend(params, param_arr, track_data)
    // params = $.zheui.assign(params, param_arr, track_data);
    for (key in params) {
